...
// Dynamic import hook for harden-react-markdown (client-side only)
...
let HardenedMarkdownCache: React.ComponentType<HardenedOptions> | null = null;
let HardenedMarkdownLoadPromise: Promise<void> | null = null;

function useHardenedMarkdown() {
  const [HardenedMarkdown, setHardenedMarkdown] =
    useState<React.ComponentType<HardenedOptions> | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    // Server-side: use ReactMarkdown directly
    ...
    if (HardenedMarkdownCache) {
      const Comp = HardenedMarkdownCache;
      setHardenedMarkdown(() => Comp);
      setIsLoading(false);
      return;
    }

    // Load once and cache
    if (!HardenedMarkdownLoadPromise) {
      HardenedMarkdownLoadPromise = import('harden-react-markdown')
        .then((module) => {
          const hardenReactMarkdown = module.default;
          HardenedMarkdownCache = hardenReactMarkdown(ReactMarkdown);
        })
        .catch((error) => {
          console.warn(
            'Failed to load harden-react-markdown, using ReactMarkdown:',
            error
          );
          HardenedMarkdownCache = ReactMarkdown;
        });
    }
...
// Visibility hook to defer heavy markdown parsing until in viewport
function useVisibility(rootMargin = '200px', threshold = 0) {
...
export const ResponseComponent = memo(
  ({
    ...
    const { HardenedMarkdown, isLoading } = useHardenedMarkdown();
    ...
    // Only enable math plugins when needed to avoid heavy processing cost
    const needsMath = useMemo(() => {
      const text = typeof parsedChildren === 'string' ? parsedChildren : '';
      // basic detection for inline ($...$) or block ($$...$$) math
      return /\$\$[\s\S]*?\$\$|(^|\s)\$[^$\n]+\$/.test(text);
    }, [parsedChildren]);

    const remarkPlugins = useMemo(() => {
      const list: Options['remarkPlugins'] = [remarkGfm];
      if (needsMath) list.push(remarkMath);
      return list;
    }, [needsMath]);

    const rehypePlugins = useMemo(() => {
      const list: Options['rehypePlugins'] = [];
      if (needsMath) list.push(rehypeKatex);
      return list;
    }, [needsMath]);
    ...
    if (lazyRender && !isVisible) {
      content = (
        <div className="my-2 h-24 w-full animate-pulse rounded-md bg-muted" />
      );
    } else if (isLoading || !HardenedMarkdown) {
      content = (
        <ReactMarkdown
