...
function isHeadingBlock(
	block: PortableBlock,
): block is Extract<PortableBlock, { _type: "block" }> {
	return (
		typeof block === "object" &&
		block !== null &&
		block._type === "block" &&
...
function blocksToPlainText(blocks: PortableBlock[] | undefined): string {
...
function blockToContextString(block: PortableBlock): string | null {
	...
	switch (block._type) {
		...
		case "callout": {
			const contentText = blocksToPlainText(
				block.content as PortableBlock[] | undefined,
			);
		...
		case "table": {
			const rows = block.rows
				?.map((row) =>
					row.cells
						?.map((cell) =>
							blocksToPlainText(cell.content as PortableBlock[] | undefined),
						)
						.filter(Boolean)
						.join(" | "),
				)
...
function LinkableHeading({
	...
	const buttonColorClass = isActive
		? "text-green-500 hover:text-green-400 group-hover/h1:bg-secondary"
		: "text-muted-foreground hover:text-green-500 group-hover/h1:bg-secondary";
